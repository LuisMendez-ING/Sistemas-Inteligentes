...
function startGame(){
	$('.hover').click(function () {
		...
		handleStyleChanges('white', '#000', 'pointer');
...
function handleStyleChanges(colorBg1, colorBg2, cursorIn) {
	$('td').css({ "background-color": colorBg1, 'cursor': cursorIn })

	$("td").hover(function () {
		$(this).css("background-color", colorBg2);
	}, function () {
		$(this).css("background-color", colorBg1);
	});
}
...
function gameOver(winner) {
	...
	if (winner)
		$('#result').css({ 'display': 'inline-block', 'background-color': '#14dc1b' }).text(winner + ' ha ganado!');
	else
		$('#result').css({ 'display': 'inline-block', 'background-color': 'crimson' }).text('Empate!');

	$('#restart').removeAttr('disabled').css('display', 'inline-block');
...
function restartGame() {
	...
	setTimeout(function () {
		...
		$('button').removeClass('seed');
		$('button').removeAttr('disabled').addClass('hover');
		$('td').removeClass();
